...
def rack_positions():
    """
    Fills out a Pandas DataFrame (positions) with calculated XY(Z) positions
    for vials and tubes based on 'N_rack' (needle reference) and 
    'P_rack' (pipette reference). This is used to automate motion commands
    when using the Chemyx-driven needle or the VIAFLO pipette.
    """
    global positions

    # Teaching coordinates for the needle
    # We offset from the taught 'N_rack' position to define a consistent 2D grid
    Nx = positions._get_value('N_rack', 'x') - 4
    Ny = positions._get_value('N_rack', 'y') + 4

    # Teaching coordinates for the pipette
    ...
    # HPLC Vials + small tubes (Pipette)
    for i in range(10):
...
teach_positions = positions  # so that new teaches append to the same DataFrame

# Generate all known rack positions (vials/tubes) based on the reference points
rack_positions()
