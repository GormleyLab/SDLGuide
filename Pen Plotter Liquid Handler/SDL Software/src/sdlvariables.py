...
def set_variables():
    ...
    dispense_mixtures = [['{}{}'.format(chr(i + 65), j + 1), 6 + 9 * i, 121 - 9 * j, False, False] for j in range(12) for i in range(8)]
    ...
    reagent_coords = pd.DataFrame({
        ...
        'Well': ['B1','C1','D1', 
                 ...
                 'A5','B5','C5','D5',
                 'A6','B6','C6','D6'],
        'X': [144,163,182,              # A1 (125,11) is kept for water, or the "filler" of choice.
              125,144,163,182,
              125,144,163,182,
              125,144,163,182,
              125,144,163,182,
    ...
    reagent_dict = pd.DataFrame({
        'reagent': [f'reagent {i+1}' for i in range(10)],  # 25 reagent names
        'Type': ['Continuous'] * 10,  # Default type for all 25 reagents
        'Group': ['No Group'] * 10,  # Default group for all 25 reagents
        'Stock': [np.nan] * 10,  # Initializing with NaN
        'Lower Bounds': [np.nan] * 10,  # Initializing with NaN
        'Upper Bounds': [np.nan] * 10  # Initializing with NaN
    })
